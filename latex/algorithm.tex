...
In this section we will present the implemented branch-and-cut algorithm, including all the components that compose the branch and cut structure: initial heuristic, branching strategies, separation algorithms, primal heuristics, etc.

\subsection{Algorithm structure}

The structure of a branch and cut algorithm can be considered as a combination of cutting planes and branch and bound schemes.

\subsubsection*{Cutting planes}

Cutting planes algorithms rely solely on valid inequalities for the problem. Given a solution for the linear relaxation of the problem\footnote{The linear relaxation of an integer linear programming problem is the same problem without integrality constraints on the variables.}, a number of cutting planes are added in order to remove the fractional solution obtained. The relaxation, along with the added cuts, is then re-solved and the process repeated until an integer-feasible solution is obtained.
...
\LOOP
	...
	\IF{solution is integral}
		\STATE return obtained solution
	\ENDIF
...
The algorithm depends on having an adequate set of cutting planes families, along with good separation heuristics, in order to find at least one valid inequality that cuts off the fractional solution on every iteration. Excessive generation of cutting planes may have the drawback that the relaxation becomes larger and larger on every iteration, require more computational effort to solve.

Note that besides the valid inequalities specific to each problem, such as the ones we have presented for \PCP{} in section \ref{sec:ineqs}, there are generic families of cutting planes that can be applied to any problem. However, problem-specific cuts usually have better performance.

\subsubsection*{Branch and bound}

The branch and bound scheme explores different solutions by setting bounds on fractional variables on every solution. The algorithm starts by solving the fractional solution, like cutting planes, but instead of trying to remove the fractional solution by adding a valid inequality, it creates two subproblems, usually by fixing a particular variable with a fractional value to either one or zero (in 0-1 integer programming models). 

Each subproblem is then resolved using the same strategy until the relaxation's solution is integer feasible, in which case that branch is closed. The algorithm runs until all nodes are explored.

Besides the \textit{branching} behaviour of the algorithm, both upper and lower \textit{bounds} for each node are considered. A global upper bound for a minimization problem is updated whenever a new integer feasible solution is found, using the solution with the lowest objective value. This bound is compared with each node's lower bound provided by the relaxation's solution, as the integer solution will always be greater than the relaxation's. Should the lower bound of the node be larger than the global upper bound, then the branch can be pruned, reducing the tree's size.
...
\WHILE{there are open nodes in the \textit{tree}}
	...
	\ELSE
		\STATE generate new subproblems by \textit{branching}
	\ENDIF
\ENDWHILE
...
The cut and branch scheme is simply an execution of the cutting planes algorithm until a certain threshold is reached (expressed in running time, number of cuts iterations or mip gap), when the generation of cutting planes is stopped and a branch and bound algorithm is executed, using the initial formulation along with all the generated cutting planes.

This algorithm usually yields better results than the previous ones, as cutting planes usually have difficulties closing the final gap, and a pure branch and bound scheme usually takes too long to solve. However, the cutting planes may cause that the relaxations in the branch and bound take too long to solve; a good balance between the two phases is extremely important for a good overall performance.

\subsubsection*{Branch and cut}

While the cut and branch algorithm applies cutting planes only at the beginning, this is, at the root of the branch and bound tree, the branch and cut version uses cutting planes throughout the whole tree. Although cuts are applied more aggressively at the root, on certain nodes in the tree more iterations of cuts are executed. 

Note that these generated cuts can use local information, exploiting the variables fixed in the node due to the branching process. In this case, cuts generated on the node are only valid on the node's subtree; otherwise, cuts generated on a node can be reused globally.

An improvement to this scheme consists in deriving integral solutions from node relaxations, therefore obtaining global upper bounds earlier during the tree traversal; the generation of these solutions is done via \textit{primal heuristics}.

As with all previously mentioned algorithms, branch and cut schemes have a number of parameters and strategies to determine. To begin with, all the separation heuristics for cutting planes iterations must be chosen adequately to quickly find a reasonable number of cuts; on the branch and bound side, node selection and branching strategies must also be determined. Also, the number of cut iterations to perform on the root and on internal nodes must be set, as well as when to apply cuts within the tree.

Throughout this section we will go over all these missing pieces in the branch and cut structure, and how we filled them in order to solve the \PCP{}.
...
There are three standard node selection strategies, all of them implemented by the ILP framework we used. In section \ref{sec:results} we experiment with the behaviour of the algorithm under different strategies:
\begin{itemize}
	...
	\defitem{BestB}{Best-bound, picks the node with... \TODO{Complete}}
\end{itemize}
...
Early experimentation with the branch and cut algorithm and with the \textsc{dsatur} algorithm has shown that for relatively small instances the latter explores all possible solutions much faster than the former, since it lacks all the overhead imposed by the different artifacts present in a full branch and cut.
...
The branch and cut algorithm was implemented in Java 1.6 using CPLEX version 12.1 both as a MIP framework and a linear programming solver for relaxations. 
...
\begin{itemize}
\item{The branch callback is invoked once the processing of a certain node has been completed in order to determine how to create the child subproblems in case the node could not be closed; inside this callback we implement the different dynamic branching strategies described in \ref{subsec:alg:branching}, as static priorities are fixed during the initialization of the problem. This callback is also used to prune the branch and cut tree once a certain number of partitions have been fixed in order to proceed with the implicit enumeration from \ref{subsec:alg:implicit}.}
...
The framework was configured to use standard branch and cut search, instead of dynamic search, to correctly determine the performance of the algorithm. Multi-core processing was also disabled.