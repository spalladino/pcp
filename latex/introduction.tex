...
\begin{itemize}
	...
	\defitem{Path}{Subset $P$ of $V$ such that each node is adjacent only to the next one in the path in the subgraph induced by $V'$ in $G$ (see figure \ref{fig:samplepath}). \samplepath}
	\defitem{Hole}{Subset $H$ of $V$ such that each node is adjacent only to the next one in the hole in the subgraph induced by $V'$ in $G$, and the last node is adjacent to the first (see figure \ref{fig:samplehole}). \samplehole}
