...
% Sample graphs
\newcommand{\sampleeclique}{
	\begin{samplegraph}{fig:sampleeclique}{Example of an extended clique.}
		\node[b] (n0) at (0,0) {};
...
\newcommand{\samplecclique}{
	\begin{samplegraph}{fig:samplecclique}{Nodes highlighted in red form a component clique.}
		\node[b] (n0) at (0,0) {};
...
\newcommand{\sampleiset}{
	\begin{samplegraph}{fig:sampleiset}{Nodes highlighted in red form a component independent set.}
		\node[b] (n0) at (0,1) {};
		\node[nhl] (n1) at (1,0) {};
...
\newcommand{\samplepath}{
	\begin{samplegraph}{fig:samplepath}{Nodes highlighted in red form a path.}
		\node[nhl] (n0) at (0,0) {};
		\node[nhl] (n1) at (1,0) {}
			edge [-] (n0);
		\node[nhl] (n2) at (2,0) {}
...
\newcommand{\samplehole}{
	\begin{samplegraph}{fig:samplehole}{Nodes highlighted in red form a hole.}
		\node[nhl] (n0) at (0,0) {};
		\node[nhl] (n1) at (2,0) {}
			edge [-] (n0);
		\node[nhl] (n2) at (2,2) {}
...
\newcommand{\sampledsatur}{
	\begin{samplegraph}{fig:sampledsatur}{Node $v_0$ has degree of saturation 3.}
		\node[n] (n0) at (0,0) {$v_0$};
		...
		\node[ngreen] (n3) at (0,2) {}
			edge [-]	(n0);

		\node[ngreen] (n4) at (1,1) {}
			edge [-]	(n0);
...
\newcommand{\samplegprime}{
\begin{figure}[h]
	\center
	\subfloat[Partitioned graph]{
		\begin{scaledsamplegraphpicture}{0.5}
			\node[b] (n0) at (0,0) {};
			\node[b] (n1) at (1,0) {};
			\node[b] (n2) at (4,4) {}
				edge [-]	(n0)
			...
			\begin{pgfonlayer}{background} 
				\node [n,fill=yellow!30,circle,minimum size=10mm,fit=(n0) (n1),label=210:$P_1$ ] {}; 
				\node [n,fill=blue!30,circle,minimum size=10mm,fit=(n2) (n3),label=10:$P_2$] {}; 
				\node [n,fill=green!30,circle,minimum size=10mm,fit=(n4) (n5),label=170:$P_3$] {}; 
			\end{pgfonlayer}
		\end{scaledsamplegraphpicture}
	}
	\subfloat[Partition graph]{
		\begin{scaledsamplegraphpicture}{0.5}
			\node[n,fill=yellow!40] (n0) at (0,0) {$v_{P_1}$};
			\node[n,fill=blue!40] (n2) at (4,4) {$v_{P_2}$}
				edge [-]	(n0);
			\node[n,fill=green!40] (n5) at (-4,4) {$v_{P_3}$};
		\end{scaledsamplegraphpicture}
	}
	\caption{\label{fig:samplegprime} Sample partitioned graph along with its partition graph.}
\end{figure}
}

\newcommand{\samplepdegree}{
\begin{samplegraph}{fig:samplepdegree}{Node $v_0$ has partition degree $\delta_P(v_0) = 2$.}
		\node[n] (n0) at (0,0) {$v_0$};
...
The \textsc{dsatur} enumeration algorithm (proposed in \cite{brelaz1979new}), which uses degree of saturation criteria for picking the next node to be colored, has proven to be one of the most efficient implicit enumeration methods for the coloring problem; having several improvements such as \cite{sewell1996improved}.
...
In this work we will consider its application to routing and wavelength assignment in WDM networks, which gives rise to the \textit{partitioned coloring problem}.
...
\begin{itemize}
	...
	\defitem{Partition Degree}{$\delta_P(v)$ is the cardinal of the partition neighbourhood of $v$ (see figure \ref{fig:samplepdegree}). \samplepdegree}
	\defitem{Color Degree}{Number of different colors used in $N(v)$ for a node $v \in V$; also degree of saturation (see figure \ref{fig:sampledsatur}). \sampledsatur}
	\defitem{Path}{Subset $P$ of $V$ such that each node is adjacent only to the next one in the path in the subgraph induced by $V'$ in $G$; formally, being $P = \{v_1, \ldots, v_k\}$, $P$ is a path if $G[P]$ has edges $[v_i, v_{i+1}]$ for $1 \leq i < k$, and no more edges between vertices of $P$  (see figure \ref{fig:samplepath}). \samplepath}
	\defitem{Hole}{Subset $H$ of $V$ such that each node is adjacent only to the next one in the hole in the subgraph induced by $V'$ in $G$, and the last node is adjacent to the first; formally, being $H = \{v_1, \ldots, v_k\}$, $H$ is a hole if $G[H]$ has edges $[v_1,v_k]$ and $[v_i, v_{i+1}]$ for $1 \leq i < k$, and no more edges between vertices of $H$ (see figure \ref{fig:samplehole}). \samplehole}
	\defitem{Component Independent Set}{Subset of $V$ such that for every pair of nodes $u,v$, $u$ is not adjacent to $v$ and they belong to different partitions (see figure \ref{fig:sampleiset}). \sampleiset}
	\defitem{Component Path}{Path $P$ in $G$ that verifies that every node in $P$ belongs to a different partition.}
	\defitem{Component Hole}{Hole $H$ in $G$ that verifies that every node in $H$ belongs to a different partition.}
	\defitem{Component Clique}{Clique $K$ in $G$ that verifies that every node in $K$ belongs to a different partition (see figure \ref{fig:samplecclique}). \samplecclique}
	\defitem{Extended Clique}{Subset of $V$ such that for every pair of nodes $[u,v]$, either $u$ is adjacent to $v$, or $u$ and $v$ are contained in the same partition (see figure \ref{fig:sampleeclique}). \sampleeclique }
	\defitem{Partition Graph}{The \textit{partition graph} $G'$ of a partitioned graph $G$ is a standard graph $G'=<V',E'>$ in which every node $v'_k \in V'$ corresponds to a partition $P_k \in P$, and two nodes $v'_i,v'_j \in V'$ are adjacent if and only if every node in $P_i$ in $G$ is adjacent to every node in $P_j$ (see figure \ref{fig:samplegprime}). \samplegprime}
\end{itemize}