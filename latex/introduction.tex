...
\begin{figure}[h]
	...
	\begin{tikzpicture} 
		...
		\node[n] (n3) at ( 4,4) {$t_1$}
			edge [-]	(n2)
			edge [-]	(n5)
			edge [<-, bend right, draw=red] node[auto] {\textcolor{red}{$R_1$}} 	(n0)
			edge [<-, bend left, draw=red] node[auto,swap] {\textcolor{red}{$R_2$}} (n0)
		;
		...
		\node[n] (n7) at ( 6,2) {$t_2$}
			edge [-]	(n5)
			edge [<-, bend right, draw=blue] node[auto] {\textcolor{blue}{$R_3$}}  (n6)
			edge [<-, bend left, draw=blue]	node[auto,swap] {\textcolor{blue}{$R_4$}} (n6) 
		;
	...
	\end{tikzpicture} 
		...
		\caption{Sample network in which connections $s_1 \rightarrow t_1$ and $s_2 \rightarrow t_2$ are to be implemented. Potential routes $R_1, R_2$ are proposed for the first, while routes $R_3, R_4$ are proposed for the second one. The corresponding partitioned graph is presented in figure \ref{fig:solvedamplerouting}.}
	\end{figure}
...
\begin{figure}[h]
		...
		\begin{tikzpicture} 
			...
			\node[n,fill=black!25] (n3) at ( 5,2) {$R_4$};
